...
const cleanCart = async (req, res) => {
  try {
    const token = await getValidToken(req);
    const { id } = jwt.verify(token, process.env.SECRETJWTKEY);
    const { users_permissions_user } = req.body;
    if (id !== users_permissions_user) {
      return res.status(HTTP_UNAUTHORIZED).json({
        error: UNAUTHORIZED_OPERATION,
      });
    }
    await Cart.deleteMany({ users_permissions_user: users_permissions_user });
    return res.json({
...
const removeFromCart = async (req, res) => {
  try {
    ...
    const itemFound = await Cart.findOne({ _id: req.body.itemCartToRemove }).where({
      users_permissions_user: id,
    });
